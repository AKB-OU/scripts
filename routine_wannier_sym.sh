...
echo "set \"write_dmn = .true.\" in pw2wan.in"
echo "set \"site_symmetry = .true.\" in seedname.win"
echo
...
if [[ $(awk '/write_dmn.*.true.$/' ${FNAME}.pw2wan.in | sed 's/^ *\| *$//') =~ ^write_dmn.*.true.$ ]]; then
	echo "we found \"write_dmn = .true.\""
else
	echo "\"write_dmn = .true.\" not found"
        exit 1
fi

if [[ $(awk '/site_symmetry.*.true.$/' ${FNAME}.win | sed 's/^ *\| *$//') =~ ^site_symmetry.*.true.$ ]]; then
	echo "we found \"site_symmetry = .true.\""
else
	echo "\"site_symmetry = .true.\" not found"
        exit 1
fi

if [[ $(awk '/symmetrize_eps*/' ${FNAME}.win | sed 's/^ *\| *$//') =~ ^symmetrize_eps* ]]; then
	echo "we found \"$(awk '/symmetrize_eps*/' ${FNAME}.win)\""
else
	echo "\"symmetrize_eps\" not found"
        exit 1
fi

if [[ $(awk '/dis_froz_*/' ${FNAME}.win | sed 's/^ *\| *$//') =~ ^dis_froz_* ]]; then
	echo "unset frozen window"
        exit 1
fi
